...
const all_role_details = read_json_file('./_mid/roles_details.raw.json');
...
const all_skill_hant = JSON.parse(converter_cn2tw(JSON.stringify(
...
outputJSON({
	json: all_skill_hant,
	fn: './_mid/__skills.hant.json',
	cn2tw: true,
});
outputJSON({
	json: unique_all_skill,
	fn: './_mid/__uni_skills.hant.json',
	cn2tw: true,
});
...
for (const sss of subskills_whitelist) {
	for (const subskill_name of sss.sub) {
		...
		const _res = await fetch_with_cached({
			...
			cached_path: `bili/绝学/${cn_name}.res.json`,
			is_json: true,
