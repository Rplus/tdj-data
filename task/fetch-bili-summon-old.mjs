...
const raw = await fetch_with_cached({
	...
	cached_path: 'bili-summon-list.res.json',
	is_json: true,
	ignore_cached: FORCE_FETCH,
});

// // 一次性 fetch all summons with props
// const res = await fetch_with_cached({
// 	url: 'https://wiki.biligame.com/tdj/api.php?action=ask&query=[[Category:召唤物]]|?名称|?天赋|?射程|?属性|?属相|?移动|?绝学|?职业&format=json',
// 	cached_path: `bili_summons.json`,
// 	is_json: true,
// 	ignore_cached: FORCE_FETCH,
// });
...
outputJSON({
	...
	fn: './_mid/__bili_summon_names.json',
	cn2tw: false,
});
...
const bili_summon_rawres = await Promise.all(
	bili_summon_names.map(async (summon_name) => {
		return fetch_limit(() => fetch_with_cached({
			...
			cached_path: `bili/${summon_name}.res.json`,
			is_json: true,
			ignore_cached: FORCE_FETCH,
		}))
	})
...
for (const summon of bili_summon_raw_data) {
	...
	const inherent_res = await fetch_with_cached({
		...
		cached_path: `bili/天赋/${inherent_name}.res.json`,
		is_json: true,
		ignore_cached: FORCE_FETCH,
	});
	...
	for (const skill_name of skill_names) {
		const _skill_res = await fetch_with_cached({
			...
			cached_path: `bili/绝学/${skill_name}.res.json`,
			is_json: true,
			ignore_cached: FORCE_FETCH,
		});
		...
		if (_skill_desc.includes('切换') && _skill_desc.match(/「[^」]+」/)) {
			...
			for (let subskill_name of subskills_name) {
				const _subskill_res = await fetch_with_cached({
					...
					cached_path: `bili/绝学/${subskill_name}.res.json`,
					is_json: true,
					ignore_cached: FORCE_FETCH,
				});
...
outputJSON({
	...
	fn: `./_mid/_bili-summon-data.res.json`,
	cn2tw: false,
});
outputJSON({
	json: matched_subskills,
	fn: `./_mid/__bili-matched_subskills.json`,
	cn2tw: false,
});
